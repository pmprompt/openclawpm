...
class EnvPreflight
{
    ...
    public static function forProvisioning(string $repoRoot): array
    {
        ...
        $env = [
            ...
            'OPENCLAW_VERSION' => getenv('OPENCLAW_VERSION') ?: 'latest',
    ...
    private static function writeEnvFile(string $repoRoot, array $env): void
    {
        ...
        foreach (['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'OPENROUTER_API_KEY', 'OPENCLAW_MODEL_PRIMARY', 'OPENCLAW_VERSION', 'PM_SKILLS_REPO', 'PM_SKILLS_REF'] as $k) {
            $v = $env[$k] ?? '';
            if ($v === null) {
                $v = '';
            }
